...
| Nintendo DS | ✅ | ⚠️ | ❌ | **Partial, and deliberately so.** Boots `.nds` test programs and hand-assembled ROMs, runs both CPUs, draws both screens in 2D and 3D, plays its sixteen sound channels, and keeps saves. BIOS calls and cartridge DMA are not wired, so commercial ROMs are not supported. Held to a lower accuracy bar than the other three; expect some games to misbehave. See below for exactly what is missing |
